...
function md5Simple(input: string): string {
  ...
  const convertToWordArray = (str: string): number[] => {
    ...
    if (i < strLen) {
      let word = str.charCodeAt(i)
      if (i + 1 < strLen) word |= str.charCodeAt(i + 1) << 8
      if (i + 2 < strLen) word |= str.charCodeAt(i + 2) << 16
      wordArray.push(word)
    }
  ...
  const strUtf8 = utf8Encode(input)
  const x = convertToWordArray(strUtf8)
