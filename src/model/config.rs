...
mod tests {
    ...
    #[test]
    fn test_model_refresh_config_default() {
        let config = ModelRefreshConfig::default();
        assert_eq!(config.enabled, true);
        assert_eq!(config.interval_seconds, 7200);
        assert_eq!(config.account_filter_cache_ttl_seconds, 300);
    }

    #[test]
    fn test_config_with_model_refresh() {
        let json = r#"{
            "host": "0.0.0.0",
            "port": 8080,
            "modelRefresh": {
        ...
        assert_eq!(config.model_refresh.enabled, false);
        assert_eq!(config.model_refresh.interval_seconds, 3600);
        assert_eq!(config.model_refresh.account_filter_cache_ttl_seconds, 600);
    }

    #[test]
    fn test_config_without_model_refresh_uses_defaults() {
        let json = r#"{
            "host": "0.0.0.0",
            "port": 8080
        }"#;
        ...
        let config: Config = serde_json::from_str(json).unwrap();
        assert_eq!(config.model_refresh.enabled, true);
        assert_eq!(config.model_refresh.interval_seconds, 7200);
        assert_eq!(config.model_refresh.account_filter_cache_ttl_seconds, 300);
    }

    #[test]
    fn test_config_default_has_model_refresh() {
